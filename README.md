...
- **RFP 기반 제안서 초안 생성 AI Agent 구현**
  ...
  - PDF, PPTX, CSV 문서에서 텍스트를 추출하고 전처리·청킹 과정을 거쳐 검색 가능한 데이터 구조로 정리
  - ChromaDB와 sentence-transformers 기반 제안서 검색 인덱스를 구축하고, RFP 기반 유사 제안서 검색 API 구현
  - FastAPI 기반으로 RFP 분석, 인덱스 재구축, 유사 제안서 검색, 초안 생성, Word/PPT export, health check API 구현
  - Streamlit UI를 통해 RFP 분석부터 초안 생성, 문서 export까지의 사용 흐름 연결
...
- **자사 솔루션 QA 및 개발팀 커뮤니케이션**
  - 내부 CRM 솔루션의 화면 오류, 요청·응답 데이터 불일치 등 품질 개선 지점 검증
  - 브라우저 개발자 도구로 요청 URL, 응답값, 오류 코드, 발생 조건을 확인
  - 이슈 발생 화면, 재현 조건, 기대 동작과 실제 동작을 구분해 개발팀이 재현 가능한 QA 리포트로 정리
